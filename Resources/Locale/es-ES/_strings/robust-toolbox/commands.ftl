...
cmd-parse-failure-bool = { $arg } no es un valor booleano válido.
...
cmd-help-desc = Muestra ayuda general o específica de un comando.
cmd-help-help =
...
cmd-help-no-args = Para obtener ayuda sobre un comando específico, usa 'help <command>'. Para consultar todos los comandos disponibles, usa 'list'. Para buscar entre ellos, usa 'list <filter>'.
...
cmd-savemap-desc = Serializa el mapa en el disco. No guarda un mapa después de inicializarlo salvo que se fuerce.
...
cmd-loadmap-help = Uso: loadmap <MapID> <Path> [x] [y] [rotación] [uids coherentes]
cmd-loadmap-nullspace = No se puede cargar 0 en el mapa.
cmd-loadmap-exists = El mapa { $mapId } ya existe.
cmd-loadmap-success = El mapa { $mapId } se cargó desde { $path }.
...
cmd-cldbglyr-help =
    Uso: cldbglyr <layer>: alterna <layer>
...
cmd-loadgrid-help = Uso: loadgrid <MapID> <Path> [x y] [rotación] [conservar UIDs]
...
cmd-mapinit-desc = Inicia el mapa indicado.
...
cmd-testbed-help = Uso: testbed <mapid> <test>
...
cmd-kick-help = Uso: kick <PlayerIndex> [<Reason>]
# Spin command
cmd-spin-desc = Hace que la entidad gire. Por defecto, la entidad es la madre del jugador conectado.
cmd-spin-help = Uso: spin <velocidad> [resistencia] [entityUid]
...
cmd-spawn-help = Uso: spawn <prototype> O spawn <prototype> <ID de entidad relativa> O spawn <prototype> <x> <y>
...
cmd-scale-help = Uso: scale <entityUid> <float>
